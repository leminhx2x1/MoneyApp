...
abstract class TransactionRepository {
  ...
  Future<StateData> updateTransaction(
      Transaction transaction, Account? oldWallet);

  Future<StateData> deleteTransaction(String? transactionId, Account account);
  ...
  Future<StateData> deleteTransaction(
    ...
    return _getUser() != null
        ? await _transactionRemoteDataSource.deleteTransaction(
            _getUser()!.id, transactionId, account)
        : _errorLogin();
  }

  @override
  Future<StateData> updateTransaction(
      Transaction transaction, Account? oldWallet) async {
    return _getUser() != null
        ? await _transactionRemoteDataSource.editTransaction(
            _getUser()!.id, transaction, oldWallet)
        : _errorLogin();
  }
      ...
      case SpendLimitType.WEEKLY:
        {
          ...
          transactions
              ...
              .toList()
              .forEach((element) {
            if (element is Transaction) result += element.amount ?? 0;
          });
      ...
      case SpendLimitType.MONTHLY:
        {
          transactions
              ...
              .toList()
              .forEach((item) {
            if (item is Transaction) result += item.amount ?? 0;
          });
          return StateData.success(result);
        }
